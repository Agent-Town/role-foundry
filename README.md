...
Those files come from a real `runner_bridge.autoresearch_alpha` execution on this branch. They now show the first narrow **mixed-execution** proof lane: baseline + candidate-teacher remain `LocalReplayRunner` / zero-secret replay, while candidate-student carries one real `--live-public-smoke` mutation plus its tracked verifier slice. They still do **not** imply sealed evaluation, independent isolation, certification, tamper-proofing, audit, or native Clawith parity beyond that explicitly exported slice.
