...
These slices are real. This branch now has an executable **public alpha loop**, evidence that the local private-holdout lane can run honestly with full replacement coverage and a local-only pre-run manifest commitment artifact before stage execution, and a named `claude_vibecosystem` external-executor beta seam whose backend provenance now threads through `artifact-bundle.json`, candidate/evaluation receipts, per-stage alpha receipts, and `sealing_receipt.execution_backend`. The claim ceiling still stops at backend provenance / claim-boundary evidence on top of local private-holdout alpha execution plus a non-destructive external-executor beta stub; sealed/certified/tamper-proof claims remain blocked.
...
- The pre-run manifest commitment and backend provenance surfaces improve local auditability/operator correlation plus backend claim-boundary evidence only. They do **not** create publication, witnessing, signing, tamper-proofing, certification, or independent audit.
