...
Roundtrip proof references now use stable tracked paths under `submission/`.
...
- portable reply excerpt: `submission/roundtrip-proof/final-reply.txt`
...
- reviewers can inspect the tracked portable bundle without needing the maintainer-local raw receipt trees
- raw receipt dirs remain maintainer-local and indexed only for deep revalidation
...
- [ ] Confirm `submission/conversation-log.md`, `submission/evidence-proof-manifest.json`, and `submission/submission-metadata.json` all record the same final review branch + commit.
