...
class PublicBenchmarkPackPhaseBTests(unittest.TestCase):
    ...
    def test_B004_public_teacher_split_integrity(self):
        ...
        self.assertEqual(metric["included_student_visible_family_count"], 6)
        self.assertEqual(metric["blocked_teacher_only_family_count"], 3)
    ...
    def test_B006_promotion_readiness_clarity(self):
        ...
        self.assertEqual(readiness_metric["family_readiness_coverage_count"], 9)
        self.assertEqual(
        ...
        self.assertEqual(
            self.episode_registry["coverage"]["family_readiness_coverage_count"], 9
        )
        self.assertEqual(
        ...
        dataset_doc_text = DATASET_DOC.read_text().lower().replace("**", "")
        ...
        self.assertIn(
            f"{len(self.pack_episodes)} public episodes", dataset_doc_text
        )
        self.assertIn(
            f"{len(self.pack['included_family_ids'])} benchmark-ready public families",
            dataset_doc_text,
        )
        self.assertIn(
            f"{len(self.pack_episodes)}/{len(self.pack_episodes)} provenance mappings",
